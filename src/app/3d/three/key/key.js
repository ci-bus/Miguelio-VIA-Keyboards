...
export class Key {
    constructor(options) {
        ...
        this.colorway = ColorUtil.getColorway();
        this.previousState = KEYSTATES.INITIAL;
        this.is_iso_enter = this.key.code === "KC_ENT" && (this.key.s == "ISO_ENTER" || this.key.h == 2);
    ...
    get swatch() {
        let group =
            KeyUtil.isMod(this.key) && "mods" in this.colorway.swatches
                ? "mods"
                : "base";
        let override = this.colorway.override
            ? this.colorway.override[this.key.code]
            : "";
        return (
